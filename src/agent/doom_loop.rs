...
mod tests {
    ...
    fn test_pattern_equality() {
        let input1 = serde_json::json!({"path": "test.txt", "limit": 100});
        let input2 = serde_json::json!({"path": "test.txt", "limit": 100});
        let input3 = serde_json::json!({"path": "other.txt", "limit": 100});
    ...
    fn test_different_inputs_no_loop() {
        ...
        for i in 0..5 {
            let input = serde_json::json!({"path": format!("test{}.txt", i)});
            assert!(detector.record("Read", &input).is_none());
        }
