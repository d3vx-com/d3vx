...
/// prompt.
///
/// `agent_id` is the stable id this agent uses when claiming tasks or
/// sending messages. The caller is responsible for ensuring it is
/// unique within the coordination root.
pub fn coordination_preamble(agent_id: &str) -> String {
    format!(
        "# Coordination context\n\
         \n\
         You are agent `{agent_id}` in a shared coordination space with other\n\
         agents. Use these tools to cooperate:\n\
         \n\
